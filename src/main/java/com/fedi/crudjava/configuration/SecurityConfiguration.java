package com.fedi.crudjava.configuration;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    @Autowired
    private DataSource dataSource; // DataSource contient des informations sur ma base ( url , root et mt d pass)
    @Value("${spring.queries.users-query}")  //  la requette SQL est defenie dans application.properties
    private String usersQuery; // requete pour obtenir tous informations des users
    @Value("${spring.queries.roles-query}") //  la requette SQL est defenie dans application.properties
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests().antMatchers("/").permitAll() // accès pour tous users
                ...
                .passwordParameter("password").and().logout().logoutRequestMatcher(new AntPathRequestMatcher("/logout")) // route de deconnexion ici/ logout
